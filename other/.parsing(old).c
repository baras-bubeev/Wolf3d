...
/*   Updated: 2020/12/13 23:52:55 by mpowder          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_texture(char *src, char **dst)
{
	...
	(ft_strlen(src) < 3) ? ft_exit(1, NOT_VALID_FILE) : 0;
	(*(++src) != ' ') ? ft_exit(1, NOT_VALID_FILE) : 0;
	while (*src && *src == ' ')
		src++;
...
static void	ft_num(char *src, int *dst, char c)
{
	...
	while (i < max)
	{
		...
		while (*src == ' ')
			src++;
		(!ft_isdigit(*src)) ? ft_exit(1, NOT_VALID_FILE) : 0;
		dst[i] = *src - '0';
